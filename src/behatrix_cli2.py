...
def permutations_test(nrandom, sequences, behaviours, exclusion_list, block_first, block_last, conta_obs):
    """
    permutation test
    returns:
    count_tot
    ...
    def strings_permutation(space: list, sequences, exclusion_list, block_first, block_last):
        """
        ...
        for seq in sequences:
            ...
            for c in seq[int(block_first):len(seq) - int(block_last)]:
                ...
                if element in exclusion_list:
                    ...
                    lspazio3 = [x for x in lspazio3 if x not in exclusion_list[element]]
    ...
    for sequence in sequences:
        ...
        sequences_num.append([behaviours.index(x) for x in sequence])
        space_num += [behaviours.index(x) for x in sequence[int(block_first):len(sequence) - int(block_last)]]


    exclusion_list_num = {}
    for behav in exclusion_list:
        exclusion_list_num[behaviours.index(behav)] = []
        for excluded_behav in exclusion_list[behav]:
            exclusion_list_num[behaviours.index(behav)].append(behaviours.index(excluded_behav))
    ...
    while True:
        ...
        permuted_sequences = strings_permutation(space_num, sequences_num, exclusion_list_num, block_first, block_last)
...
def main():
    ...
    if not args.quiet:
        ...
        for behaviour in behaviours:
            ...
            print("{behaviour}\t{freq}\t{countBehaviour} / {tot_nodes}".format(behaviour=behaviour,
